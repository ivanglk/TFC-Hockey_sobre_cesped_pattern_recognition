...
print("--- INICIANDO SISTEMA V6: HÍBRIDO + CINEMÁTICA VERTICAL + REPRODUCTOR ---")
...
estado_posesion = "Indefinido" # "Ataca_Arriba" o "Ataca_Abajo"
registro_eventos = [] 

# Parámetros Lucas-Kanade (Flujo Óptico)
...
salto_solicitado = True # Fuerza a leer un frame inicial
...
def click_botones(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Lógica de las coordenadas de los botones (dibujados abajo)
        if 550 <= y <= 590:
            if 10 <= x <= 80:    # << 5s
                cap.set(cv2.CAP_PROP_POS_FRAMES, max(0, frame_actual - 5 * fps_video))
                salto_solicitado = True
            elif 90 <= x <= 160: # < 1s
                cap.set(cv2.CAP_PROP_POS_FRAMES, max(0, frame_actual - 1 * fps_video))
                salto_solicitado = True
            elif 170 <= x <= 270: # PLAY/PAUSE
                estado_reproduccion = "PAUSE" if estado_reproduccion == "PLAY" else "PLAY"
            elif 280 <= x <= 350: # > 1s
                cap.set(cv2.CAP_PROP_POS_FRAMES, frame_actual + 1 * fps_video)
                salto_solicitado = True
            elif 360 <= x <= 430: # >> 5s
                cap.set(cv2.CAP_PROP_POS_FRAMES, frame_actual + 5 * fps_video)
                salto_solicitado = True

# ==========================================
# 4. FUNCIONES GEOMÉTRICAS (V5)
# ==========================================
...
# ==========================================
# 5. LÓGICA DE HORIZONTE (Perspectiva Cabecera)
# ==========================================
def inferir_zona_horizonte(objetos_yolo):
    """
    Identifica la zona del campo analizando la altura en pantalla (eje Y) 
    de las anclas detectadas por la IA.
    """
    for obj in objetos_yolo:
        if obj["clase"] == "goal":
            # Si el arco está en la mitad inferior de la pantalla, es el Local.
            return "Z1_ArcoLocal_25yd" if obj["cy"] > (VIDEO_H / 2) else "Z4_25yd_ArcoVisita"
            
    for obj in objetos_yolo:
        if obj["clase"] == "25yd line":
            # Si vemos una 25yd sin arco, inferimos por su altura
...
# ==========================================
cv2.namedWindow("Dashboard Analitico V6")
cv2.setMouseCallback("Dashboard Analitico V6", click_botones)
...
while cap.isOpened():
    if estado_reproduccion == "PLAY" or salto_solicitado:
        ...
        # --- A. CINEMÁTICA VERTICAL (TILT PTZ) ---
        movimiento_y = 0
        if color_gris_previo is not None and puntos_previos is not None and len(puntos_previos) > 0:
            ...
            if puntos_actuales is not None:
                ...
                if len(p_nuevos) > 0:
                    # EJE Y: Si la cámara sube (Tilt Up), el fondo baja (Y positivo)
        ...
        for box in results[0].boxes:
            ...
            if "goal" in nombre_clase:
                cv2.circle(annotated_frame, (cx, cy), 8, (255, 0, 0), -1) 
        ...
        # --- C. GEOMETRÍA HÍBRIDA (Nodos Virtuales) ---
        ...
        # --- D. LÓGICA DE RECUPERACIÓN (MÁQUINA DE ESTADOS) ---
        zona_actual = inferir_zona_horizonte(objetos_detectados)
        evento_trigger = None

        # Tilt Up (Fondo baja) -> Ataque a Visita. Tilt Down (Fondo sube) -> Ataque a Local
        if tendencia_flujo_y > 1.5: 
            nuevo_estado = "Ataca_Arriba (Visita)"
        elif tendencia_flujo_y < -1.5:
            nuevo_estado = "Ataca_Abajo (Local)"
        else:
            nuevo_estado = estado_posesion

        # Detección de Turnover
        if nuevo_estado != estado_posesion and estado_posesion != "Indefinido" and zona_actual in metricas_recuperacion:
            metricas_recuperacion[zona_actual] += 1
            evento_trigger = f"RECUPERACION: {zona_actual}"
    ...
    # --- E. DIBUJAR INTERFAZ (UI) EN CADA FRAME ---
    if annotated_frame is not None:
        display_frame = annotated_frame.copy()
        
        # Panel Superior: Datos Analíticos
        ...
        cv2.putText(display_frame, f"FILTRO HORIZONTE: {zona_actual}", (20, 55), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
        
        # Alerta visual de recuperación
        ...
        # Panel Inferior: Botonera de Reproducción
        ...
        cv2.imshow("Dashboard Analitico V6", display_frame)

    # Controles por teclado adicionales
    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'): break
    elif key == ord(' '): # Espacio para Play/Pause
        estado_reproduccion = "PAUSE" if estado_reproduccion == "PLAY" else "PLAY"
...
# ==========================================
# 8. EXPORTAR DATOS PARA LA TESIS
# ==========================================
df_resultados = pd.DataFrame(registro_eventos)
df_resultados.to_csv("recuperaciones_hockey.csv", index=False)
print("\n--- PROCESAMIENTO FINALIZADO ---")
print("Se ha generado el archivo 'recuperaciones_hockey.csv' con la tabla de eventos.")
