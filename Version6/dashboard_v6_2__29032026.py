...
# ==========================================
# 6. INICIALIZACIÓN DE VENTANA Y CALIBRACIÓN
# ==========================================
cv2.namedWindow("Dashboard Analitico V6.1")
cv2.setMouseCallback("Dashboard Analitico V6.1", click_botones)
model = YOLO(path_modelo)
...
if success_init:
    ...
    # Dibujar las 4 franjas esquemáticas del Filtro de Horizonte
    # Z4 (Visita) - Arriba
    cv2.rectangle(overlay, (0, 0), (VIDEO_W, int(VIDEO_H * 0.25)), (0, 0, 255), -1) 
    # Z3 (Medio Visita)
    cv2.rectangle(overlay, (0, int(VIDEO_H * 0.25)), (VIDEO_W, int(VIDEO_H * 0.50)), (0, 165, 255), -1) 
    # Z2 (Medio Local)
    cv2.rectangle(overlay, (0, int(VIDEO_H * 0.50)), (VIDEO_W, int(VIDEO_H * 0.75)), (0, 255, 255), -1) 
    # Z1 (Local) - Abajo
    cv2.rectangle(overlay, (0, int(VIDEO_H * 0.75)), (VIDEO_W, VIDEO_H), (0, 255, 0), -1)
    
    # Aplicar transparencia (Alpha blending) al 30%
    cv2.addWeighted(overlay, 0.3, frame_calib, 0.7, 0, frame_calib)
    
    # Textos de la pantalla
    cv2.rectangle(frame_calib, (0, int(VIDEO_H/2 - 40)), (VIDEO_W, int(VIDEO_H/2 + 40)), (20, 20, 20), -1)
    cv2.putText(frame_calib, "CALIBRACION DE ZONAS (FILTRO DE HORIZONTE)", (80, int(VIDEO_H/2 - 5)), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 255), 2)
    ...
    # Etiquetas de zonas
    cv2.putText(frame_calib, "Z4: 25yd a Arco Visita", (20, int(VIDEO_H * 0.12)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    cv2.putText(frame_calib, "Z3: 50yd a 25yd Visita", (20, int(VIDEO_H * 0.37)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    # Bucle de espera
    while True:
        cv2.imshow("Dashboard Analitico V6.1", frame_calib)
        key_init = cv2.waitKey(1) & 0xFF
        if key_init == ord(' '): # Rompe el bucle inicial al presionar espacio
            break
        elif key_init == ord('q'):
            cap.release()
...
# Reseteamos el video al frame 0 para empezar limpios
