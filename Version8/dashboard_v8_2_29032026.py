#==================================
# Esta es la versión más fiable hasta el momento. 17 33 Hs del 29032026
...
print("--- INICIANDO SISTEMA V8.1: CRONÓMETRO MM:SS + MÉTRICAS SEPARADAS POR EQUIPO ---")
...
# ==========================================
# NUEVO: Diccionario anidado para separar las métricas
...
while cap.isOpened():
    if estado_reproduccion == "PLAY" or salto_solicitado:
        ...
        # NUEVO: Dibujo del Tablero de Métricas Separadas
        cv2.putText(display_frame, "RECUPERACIONES", (VIDEO_W - 270, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Columna Local
        cv2.putText(display_frame, "LOCAL", (VIDEO_W - 270, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
        y_offset = 55
        for zona, conteo in metricas_recuperacion["Local"].items():
            cv2.putText(display_frame, f"{zona[:2]}: {conteo}", (VIDEO_W - 270, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)
            y_offset += 15

        # Columna Visita
        cv2.putText(display_frame, "VISITA", (VIDEO_W - 150, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
        y_offset = 55
        for zona, conteo in metricas_recuperacion["Visita"].items():
            cv2.putText(display_frame, f"{zona[:2]}: {conteo}", (VIDEO_W - 150, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 0, 255), 1)
            y_offset += 15
...
if not df_resultados.empty: 
    # Reordenamos las columnas para que el Cronómetro sea lo primero que vea el DT
