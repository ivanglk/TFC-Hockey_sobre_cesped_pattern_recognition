...
print("--- INICIANDO SISTEMA V8.1: MASTER RELEASE FINAL (CON IDENTIFICACIÓN DE EQUIPO) ---")
...
# --- CONTROL DE INTERFAZ Y TÁCTICA ---
mostrar_anotaciones_ingeniero = True # Alternar entre Ingeniero y DT
cambio_de_lado = False # Control para el segundo tiempo (Invierte Z1 y Z4)
...
def dibujar_minimapa(lienzo, jugadores_info, centro_disputa, zona_actual, invertido):
    ...
    color_z4, color_z3 = (0, 0, 255) if zona_actual in ["Z4_25yd_ArcoVisita", "Z1_ArcoLocal_25yd"] and (not invertido and zona_actual=="Z4_25yd_ArcoVisita" or invertido and zona_actual=="Z1_ArcoLocal_25yd") else (255, 255, 255), (0, 165, 255) if zona_actual in ["Z3_50yd_25yd_Visita", "Z2_25yd_50yd_Local"] else (255, 255, 255)
...
# ==========================================
cv2.namedWindow("Dashboard Analitico V8.0")
cv2.setMouseCallback("Dashboard Analitico V8.0", click_botones)
...
while cap.isOpened():
    if estado_reproduccion == "PLAY" or salto_solicitado:
        ...
        for box in results[0].boxes:
            ...
            if "player" in nombre_clase:
                ...
                if x2 > x1 and y2 > y1:
                    ...
                    team_color = (255, 255, 255) if mean_val > 150 else (0, 0, 0)
        ...
        # --- D. MOTOR SEMÁNTICO, MÁQUINA DE ESTADOS E IDENTIFICACIÓN DE EQUIPO ---
        ...
        if nuevo_estado != estado_posesion:
            if estado_posesion != "Indefinido": 
                # LÓGICA DEDUCTIVA: El equipo que inicia el nuevo ataque es el que acaba de recuperar la bocha
        ...
        c_text = "CAMBIO LADO (ON)" if cambio_de_lado else "CAMBIO LADO (OFF)"
        cv2.rectangle(display_frame, (cx1, VIDEO_H - 40), (cx2, VIDEO_H - 10), c_color, -1)
        cv2.putText(display_frame, c_text, (cx1 + 5, VIDEO_H - 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)

        cv2.imshow("Dashboard Analitico V8.0", display_frame)
